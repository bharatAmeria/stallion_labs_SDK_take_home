...
export class ModelNotFoundError extends BitNetError {
  ...
  constructor(modelId: string) {
    super(
      'MODEL_NOT_FOUND',
      `Model "${modelId}" is not cached locally. Call downloadModel() first.`
    );
...
export class ConcurrencyLimitError extends BitNetError {
  ...
  constructor(activeRequests: number, limit: number) {
    super(
      'CONCURRENCY_LIMIT',
      `Engine is at max concurrency (${activeRequests}/${limit} active requests). ` +
...
export class EngineNotInitializedError extends BitNetError {
  constructor() {
    super(
      ...
      'No model is loaded. Call BitNet.loadModel(modelId) before generating text.'
    );
...
export class NativeError extends BitNetError {
  ...
  constructor(nativeCode: number, message: string) {
